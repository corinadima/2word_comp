...
--
-- Uncrossed Full Lexical composition model
--
-- 

local UncrossedFullLex, parent = torch.class('torch.UncrossedFullLex', 'torch.CompositionModel')

function UncrossedFullLex:__init(inputs, outputs, vocab_size)
	parent.__init(self)
	self.inputs = inputs
	self.outputs = outputs
	self.vocab_size = vocab_size
	self.name = "uncrossed_fulllex"
	self.isTrainable = true
	self.iteratorType = 'quad'
end

function UncrossedFullLex:architecture(config)
	parent:architecture(config)
	
	print("# UncrossedFullLex; vectors u,v as input; matrices A_w learned for each word individually; p = g(W[A_u_idx * u; A_v_idx * v] + b) ")
	...
	local mul_1 = nn.MM()({reshape_Au, u})
	local mul_2 = nn.MM()({reshape_Av, v})
...
function UncrossedFullLex:data(trainSet, devSet, testSet, fullSet, cmhEmbeddings)
