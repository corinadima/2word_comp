...
local TableBatchIterator, parent = torch.class('torch.TableBatchIterator', 'torch.BatchIterator')

function TableBatchIterator:initialize(args)
	self.data = args.data
...
function TableBatchIterator:processExample(example)
	return {{example[1][1]:clone(), example[1][2]:clone()}, example[2]:clone()}
end

function TableBatchIterator:nextBatch()
